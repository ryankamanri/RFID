...
public class PureAloha
{
    ...
    public static void PlotTagCount()
    {
        ...
        for(var i = 1; i <= 250; i++)
        {
            ...
            {
                // Get The Average
                occupyRatio[j] = Do(i, 100, 100, 10000);
            }
            X[i - 1] = i;
    ...
    public static void PlotMessageLength()
    {
        const int emulateCount = 5;
        var X = new double[250];
        var Y = new double[250];
        for(var i = 1; i <= 250; i++)
        {
            ...
            {
                // Get The Average
                occupyRatio[j] = Do(100, i, 100, 10000);
            }
            X[i - 1] = i;
            Y[i - 1] = occupyRatio.Average();
        }
        ...
        plot.Plot2(X, Y)
            .AddLabel(MatlabPlot.ReadyPlot.LabelLocation.X, "Message Length")
            .AddLabel(MatlabPlot.ReadyPlot.LabelLocation.Y, "Channel Occupy Ratio")
            .AddLegend("Emulate The Relation Of Message Length And The Channel Occupy Ratio")
            .Execute();
    }
    
    public static float Do(int tagCount, int messageLength, int minWaitTime = 100, int maxWaitTime = 10000)
    {
        // Input Properties
        ...
        var occupyRatio = (float)(tagCount * messageLength) / totalTime;
